...
interface ChatPanelProps {
  // 'page': página completa en /chat (cabecera grande con título y subtítulo).
  // 'panel': dentro del panel flotante de GogginsBubble, que ya pone su propia
  //          cabecera; aquí solo se muestra el botón de borrar historial.
  variant?: 'page' | 'panel'
}

export default function ChatPanel({ variant = 'page' }: ChatPanelProps) {
  ...
  return (
    <div
      className={`flex flex-col ${
        variant === 'page'
          // Alturas ajustadas a: cabecera + padding del main (con pb-24 en móvil
          // por la barra inferior; pb-8 a partir de md, donde la barra desaparece).
          ? 'space-y-4 h-[calc(100vh-190px)] sm:h-[calc(100vh-200px)] md:h-[calc(100vh-180px)]'
          : 'space-y-3 h-full'
      }`}
    >
      {/* Header */}
